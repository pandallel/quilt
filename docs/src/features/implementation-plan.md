...
2. ⚠️ Integrate with TextSplitter for content chunking (within Processor Task)

   - ⚠️ Create MaterialCut model from text-splitter chunks (within Processor Task)
   - ⚠️ Implement error handling with fallback strategy (within Processor Task)
   - ⚠️ Handle backpressure: Listener task should `await send()` and handle potential `EventBus` lag (`RecvError::Lagged`)

3. Implement Cut event publishing (1 day)

   - Add MaterialCut event creation and publishing
   - Update material status in registry (Processing → Cut) (from Processor Task)
   - Implement error reporting for failed cuts (from Processor Task)
