...
impl MaterialRepository {
    ...
    pub async fn update_material_status(
        ...
        let material = materials.get_mut(id).ok_or_else(|| {
            RepositoryError::MaterialNotFound(id.to_string())
        })?;
...
mod tests {
    ...
    async fn test_update_material_status() {
        ...
        repo.update_material_status(&id, MaterialStatus::Cut, None).await.unwrap();
        ...
        repo.update_material_status(&id, MaterialStatus::Swatched, None).await.unwrap();
    ...
    async fn test_invalid_status_transition() {
        ...
        let result = repo.update_material_status(&id, MaterialStatus::Swatched, None).await;
        assert!(result.is_err());
    ...
    async fn test_transition_to_error() {
        ...
        repo.update_material_status(&id, MaterialStatus::Error, Some(error_msg.clone())).await.unwrap();
    ...
    async fn test_list_materials() {
        ...
        // List by status
        let discovered = repo.list_materials_by_status(MaterialStatus::Discovered).await;
        assert_eq!(discovered.len(), 1);
        
        let cut = repo.list_materials_by_status(MaterialStatus::Cut).await;
        assert_eq!(cut.len(), 1);
        
        let swatched = repo.list_materials_by_status(MaterialStatus::Swatched).await;
