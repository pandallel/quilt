...
mod integration_tests {
    use crate::materials::{
        create_channel, create_channel_with_capacity, ChannelPair, Material, MaterialChannelExt,
        MaterialMessage, MaterialRepository, MaterialStatus,
    };
    ...
    async fn test_pipeline_message_flow() {
        ...
        discovery_cutting
            .sender
            .send_message(MaterialMessage::Discovered(material.clone()))
            .await
            .unwrap();
        ...
        if let MaterialMessage::Discovered(received_material) = received_message {
            ...
            repo.update_material_status(&material_id, MaterialStatus::Cut, None)
                .await
                .unwrap();
            ...
            cutting_labeling
                .sender
                .send_message(MaterialMessage::Cut(material_id.clone()))
                .await
                .unwrap();
        } else {
        ...
        if let MaterialMessage::Cut(id) = received_message {
            ...
            repo.update_material_status(&material_id, MaterialStatus::Swatched, None)
                .await
                .unwrap();
        } else {
    ...
    async fn test_channel_backpressure() {
        ...
        let ChannelPair {
            sender,
            mut receiver,
        } = create_channel_with_capacity(2);
        ...
        sender
            .send_message(MaterialMessage::Discovered(material1))
            .await
            .unwrap();
        sender
            .send_message(MaterialMessage::Discovered(material2))
            .await
            .unwrap();
        ...
        sender
            .send_message(MaterialMessage::Shutdown)
            .await
            .unwrap();
    }
