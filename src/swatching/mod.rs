...
mod tests {
    ...
    async fn test_embedding_service_mock() {
        ...
        mock.expect_embed()
            ...
            .times(1)
            .returning(|_| Box::pin(future::ready(Ok(vec![0.1, 0.2, 0.3]))));
    ...
    #[tokio::test]
    async fn test_embedding_service_error_propagation() {
        ...
        mock.expect_embed()
            ...
            .times(1)
            .returning(|_| {
                Box::pin(future::ready(Err(EmbeddingError::ModelLoadFailed(
        ...
        let result = service.embed("generate error").await;
        ...
        // Test model error
        let result = service.embed("model error").await;
        assert!(result.is_err());
        match result {
